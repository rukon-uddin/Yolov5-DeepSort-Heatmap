...
class Annotator:
    ...
    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(12, 120, 255)):
        # Add one xyxy box to image with label
        xmax, xmin, ymax, ymin = int(box[2].item()), int(box[0].item()), int(box[3].item()), int(box[1].item())
        w = xmax - xmin
        h = ymax - ymin
        w = w // 2
        h = h // 2
        ...
        if self.pil or not is_ascii(label):
            ...
            if label:
                ...
                cv2.putText(self.im, label.split(" ")[0], (c1[0], c1[1] - 2), 0, self.lw / 3, txt_color, thickness=tf,
                            lineType=cv2.LINE_AA)
        
        cv2.circle(self.im, (cx, cy), 1, color, -1)
        return cx, cy
